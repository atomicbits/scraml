...
object CaseClassGenerator extends ScalaPlaySourceGenerator {
  ...
  private def findRecursiveFields(fields: Seq[Field], selfAndParents: List[ClassReference]): Set[Field] = {
    ...
    def isRecursiveClassReference(classReference: ClassReference): Boolean = {
      ...
      val typeParamClassReferences = classReference.typeParamValues.values.map(_.native)
  ...
  private def generateCompanionObject(sortedFields: Seq[Field],
    ...
    val formatUnLiftFields = sortedFields.map(field => ScalaPlay.fieldFormatUnlift(field, recursiveFields))
    ...
    def over22FieldFormatterBody = {
      ...
      val (fieldGroupDefinitions, fieldGroupNames): (List[String], List[String]) =
        groupedFields.zipWithIndex.map {
          case (group, index) =>
            val formatFields   = group.map(field => ScalaPlay.fieldFormatUnlift(field, recursiveFields))
