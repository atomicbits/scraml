...
import io.atomicbits.scraml.ramlparser.model.canonicaltypes.{ CanonicalName, NumberType, TypeReference }
import io.atomicbits.scraml.ramlparser.model.parsedtypes.{ ParsedNumber, ParsedType }

/**
  * Created by peter on 30/12/16.
  */
object ParsedNumberTransformer {
  ...
  def unapply(parsedTypeContext: ParsedTypeContext)
    ...
    parsed match {
      case parsedNumber: ParsedNumber => Some((NumberType, canonicalLookupHelper))
      case _                          => None
    }
  }
